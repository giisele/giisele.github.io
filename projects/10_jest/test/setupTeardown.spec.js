...
describe('quem sobreviveu?', () => {
  ...
  const fillAdventurers = () => {
    ...
    adventure.specialists.forEach((value, index) => {
      if (index !== (adventure.specialists.length - 1)) {
        aventureiros += (`${adventure.specialists[index].nome}, `);
      } else aventureiros += (`${adventure.specialists[index].nome}`);
    });
  ...
  afterEach(() => {
    const aventureiros = fillAdventurers();

   
      console.log(`Aventureiros restantes: ${aventureiros}`);
    
  });

  afterAll(() => console.log(`Aventureiro sobrevivente: ${adventure.specialists[0].nome}. Parabéns, você venceu!`));

  // o afterAll da Gabriel Feijo ficou lindo!
  // afterAll(() => console.log(`${adventure.specialists.map((nome) => nome.nome)} é um vencedor! Na vida e no jogo!`));
